...
mod tests {
    ...
    fn hnsw_vs_linear() -> Result<()> {
        ...
        for i in 0..1000 {
            ...
            let meta = Metadata { label: i.to_string(), description: None };
            db.add(i, v.clone(), meta)?;
            all_vectors.push(v);
        }
        let query = vec![0.5_f32.sin(), 1.0_f32.cos(), 1.5_f32.sin()];
        let db_results = db.search(&query, 10)?;

        let mut brute: Vec<(usize, f32)> = all_vectors
            .iter()
            .enumerate()
            .map(|(i, v)| (i, cosine_distance(&query, v)))
            .collect();
        brute.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
        let best: Vec<usize> = brute.iter().take(10).map(|x| x.0).collect();
        let found: Vec<usize> = db_results.iter().map(|r| r.id).collect();

        let matches = found.iter().filter(|id| best.contains(id)).count();
        assert!(matches >= 5);
